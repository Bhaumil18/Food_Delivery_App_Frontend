        ...
        <div className='flex justify-around border border-gray-500 rounded-3xl gap-4 py-2 px-4 items-center'>
            ...
            <Formik initialValues={{ search: '' }} onSubmit={onSubmit} onReset={onSubmit}>
                <Form className='flex flex-1 gap-4 items-center'>
                    <Field className='flex-1 bg-white px-2 outline-none' placeholder={placeholder} name='search' type='text' required={required}></Field>
                    {
                        reset ? <Button className='bg-white text-black hover:bg-black hover:text-white rounded-3xl' type='reset'>Reset</Button> : null
                    }
                    <Button className='bg-orange-500 rounded-3xl' type='submit'>Search</Button>
                </Form>
            </Formik>
        </div>
    )
